...
<div id='greenstripe' class='greenStripe'>How much would you like to borrow? <select name='todo' id='todo' style='width: 100px !important;'><option>$500</option><option>$750</option><option>$1,000</option></select></div>
<!--
####### #     # ####### ######  #     # #######
#     # #     #    #    #     # #     #    #
#     # #     #    #    #     # #     #    #
...
function clear() {echo("<div style='clear:both;'></div>");}
function left($txt) {echo("<div style='float:left;'>$txt</div>");}
function right($txt) {echo("<div style='float:right;'>$txt</div><div style='clear:both;'></div>");}
...
function field($field_name,$is_required,$field_type,$max_length,$description,$style="")
        {
		global $output;
		$output='';
		global $field_list;
	 ...
	 $value='';
        ...
        if($field_type != 'hidden'){ if ($is_required) { if ( ($style=='') or ( strpos($style,'class')=== 0 ) ) xecho("<span style=\"color: red; visibility: invisible;\">*</span>"); } }
		if($field_type == 'textarea'){
			xecho("<textarea name='$field_name' id='$field_name'");
			if($is_required){
		...
		elseif($field_type=='state')
			{
			xecho("<select name='$field_name' id='$field_name'>");
			xecho(states($value));
			xecho("</select>");
			}
		...
		elseif(strpos($field_type,'|') !== false)
			{
			xecho("$description <select name='$field_name' id='$field_name'>");
			$oarr=explode('|',$field_type);
			xecho("<option selected disabled=\"disabled\">Select</option>");
		...
		elseif($field_type == 'yesno')
			...
			xecho("<option value='zero!'>No</option>"); // stupid form will not post a 0. proxy fixes it
			xecho("</select>");
			}
		elseif($field_type == 'noyes')
			{
			xecho(" $description <select name='$field_name' id='$field_name'>");
			xecho("<option value='zero!'>No</option>"); // stupid form will not post a 0. proxy fixes it
			xecho("<option value='1'>Yes</option>");
		...
		elseif($field_type == 'hidden')
			{
            		xecho("<input type='hidden' name='$field_name' id='$field_name' value='$value'>");
        		}
		elseif($field_type == 'date')
			{
		            $txt="$description <input type='text'";
		            if($is_required)
				{
				$txt.=" required='required'";
				}
		            $txt.=" class='date' name='$field_name' id='$field_name' value='$value' $style>";
			xecho($txt);
			}
        	else{
			//<input type='text' placeholder='First Name' required='required' style='width: 187px;'>

	     if (strpos($description,':') > 0)
		{
		xecho("<div style='clear:both;'></div><div style='float:left;position:relative; top:7px;'>");
		xecho(substr($description,0,strpos($description,':')));
		xecho("</div>");
		$placeholder=substr($description,strpos($description,':')+1);
		xecho("<div style='float:right;'>");
		}
	     else
		{
		$placeholder=$description;
		xecho("<label for='$field_name'>$placeholder</label>");
		$placeholder='';
		}
            xecho("<input type='$field_type' placeholder='$placeholder'");
            if($is_required)
		{
		xecho(" required='required'");
		}
            xecho(" name='$field_name' id='$field_name' value='$value' $style>");
	     if (strpos($style,'class') != false) $style='';
	...
	if (($field_type=='text') and ($style==''))
		{
		$output="<div class='slider'>$output</div>";
...
$req="<span style=\"color: red; visibility: invisible;\">*</span>";
echo "<div id='waitspace' style=\"display:none;z-index:5;text-align:center;\"></div>";
####### ####### ######  #     #  #####  ######     #     #####  #######
#       #     # #     # ##   ## #     # #     #   # #   #     # #
#       #     # #     # # # # # #       #     #  #   #  #       #
#####   #     # ######  #  #  #  #####  ######  #     # #       #####
...
echo(field('ckm_campaign_id',true,'hidden','.$id.','Assigned by RockStarAN.com for each campaign'));
echo(field('ckm_key',true,'hidden','.$key.','Password assigned by RockStarAN.com for each campaign'));
echo(field('ip_address',true,'hidden','15','IP Address'));
echo(field('requested_amount',true,'hidden','1000',''));
echo(field('ref_url',true,'hidden','http://themoneycompany.com',''));
...
echo "<div name='formpart1' id='formpart1';display:inline;'>";
echo "<div class='formRow mobileExtraPad'>";
//echo "<h3>Personal Info</h3>";
echo(field('first_name',true,'text','25','First Name',""));
echo(field('last_name',true,'text','25','Last Name',""));
echo ('</div>');
echo("<div class='formRow'>");
echo(field('email',true,'text','60','Email Address',""));
echo(field('zip',true,'text','8','Zip Code',"class='zip'"));
echo ('</div>');
echo "<!-- agreement -->";
echo "<div class='formRow'>";
echo "<div style='width: 10%; float: left;'><input type='checkbox'></div>";
echo "<div style='width: 90%; float: left; font-size: 9pt;'>By submitting your information, you agree to the terms of our <a href='#' class='showModal' data-which='Legal'>Privacy Policy</a>, <a href='#' class='showModal' data-which='Legal'>Terms of Web Site Use</a>, and special offers.</div>";
echo "<div class='clear'></div>";
echo "</div>";
echo "<!-- get started button -->";
echo "<div style='margin-top: 20px; margin-bottom: 10px; width: 100%; text-align: center;'>";
echo "<img onclick=\"return handle('1');\" src='/get-started.png' alt='Get Started' style='margin: 0px; border: 0px; padding: 0px;'>";
echo "</div>";
echo "<!-- learn more button -->";
echo "<div style='margin-top: 5px; width: 100%; text-align: center;'>";
echo "<a href='#' class='showModal btn-green' data-which='HowItWorks' style='margin-bottom: 20px;'>Learn More</a>";
echo "</div>";
echo "</div><!--formpart1-->";
...
echo "<div name='formpart2' id='formpart2' style='display:none;'>";
echo "<div class='formRow mobileExtraPad'>";
echo "<div id='p2namespace'></div>";
echo "</div>";
echo "<div class='formRow'>";
//echo(field('gender',false,'Male|Female','','Gender',''));
echo(field('dob',false,'text','','Birth Date:01/01/1900',"class='date'"));
clear();
left(field('is_citizen',false,'yesno','','US Citizen?'));
right(field('is_military',false,'noyes','','Active/Reserve Military?'));



echo(field('ssn',false,'text','','Social Security Number:000-00-0000','class="ssn"'));
echo(field('drivers_lic',false,'text','21','Drivers License Number:'));
clear();
left('Driver License State');
if (empty($_SESSION['dlstate'])) $_SESSION['dlstate']='FL'; // return the correct state during part 1 ajax post <------------------------------- HEY!
right(field('dlstate',false,'state','','Driver License State'));
echo("</div>");
echo "<!-- next prev buttons -->";
echo "<div style='margin-top: 20px; margin-bottom: 10px; width: 100%; text-align: center;'>";
...
#       #     # #    #     #          #    #     # #    #  #       #
#       #     # #     #    #          #    #     # #     # ####### #######
echo "<div name='formpart3' id='formpart3' style='display:none;'>";
echo "<div class='formRow mobileExtraPad'>";
echo "<div id='p3namespace'></div>";
echo "</div>";
echo "<div class='formRow'>";
echo(field('primary_phone',false,'text','14','Home Phone:(000) 000-0000','class="phone"'));
echo(field('secondary_phone',false,'text','14','Alternate:(000) 000-0000','class="phone"'));
echo(field('cell_phone',false,'text','14','Cell Phone:(000) 000-0000','class="phone"'));
echo(field('primary_phone2',false,'hidden','14','Primary Phone 2:(000) 000-0000','class="phone"'));
//echo(field('cell_phone2',false,'hidden','14','Cell Phone 2:(000) 000-0000','class="phone"'));
clear();
echo(field('best_time_to_call',false,'Afternoon|Morning|Evening','','Best Time to Call'));
//echo(field('contact_time',false,'M|A|E','1','Contact Time'));
echo("</div>");
echo "<!-- next prev buttons -->";
echo "<div style='margin-top: 20px; margin-bottom: 10px; width: 100%; text-align: center;'>";
...
#       #     # #    #     #       #       #     # #     # #    #
#       #     # #     #    #       #       #######  #####  #     #
echo "<div name='formpart4' id='formpart4' style='display:none;'>";
echo "<div class='formRow mobileExtraPad'>";
echo "<div id='p4namespace'></div>";
echo "</div>";
echo "<div class='formRow'>";
echo(field('address',false,'text','100','Your Street Address'));
echo(field('address_2',false,'text','100','Suite or Apartment #'));
echo(field('csz',false,'text','100','City, State & Zip Code'));
//echo(field('city',false,'text','39','City'));
//echo(field('state',false,'FL|GA|NY','','State'));
//echo(field('country',false,'Country Abv (e.g. US, MX)','2','Country'));
echo '</div>';
echo "<div class='formRow'>";

//echo(field('timeatresidence',false,'hidden','','Time at Residence'));
clear();
//left("field('address_years',false,'Less than 1|Less than 2|More than 2','','Years','style="width:100px;"'));
//right(field('address_months',false,'Less than 6|Less than 12|More than one year','','Months','style="width:100px;"'));
echo(field('address_months',false,'Less than 6 Months|Less than 1 Year|1-2 Years|Over 2 Years','','Time at Residence',''));

echo "</div>";
echo "<div class='formRow'>";
echo(field('rent_own',false,'Rent Home|Own Home','','Do you rent or own?'));
...
#       #     # #    #     #       #        #    # #   #
#       #     # #     #    #       #       ###    #    #######
echo "<div name='formpart5' id='formpart5' style='display:none;'>";
echo "<div class='formRow mobileExtraPad'>";
echo "<div id='p5namespace'></div>";
echo '</div>';
echo "<div class='formRow'>";

echo(field('employer',true,'text','100','Employer:Company Name'));
echo(field('emp_years',false,'number','','Number of Years Employed:00','style="width:100px;"'));
echo(field('emp_address',false,'text','300','Employer Address:Work Address'));
//echo(field('emp_address_2',false,'text','100','Employer Address 2'));
//echo(field('emp_city',false,'text','50','Employer City:City'));
//echo(field('emp_state',false,'text','2','Employer State:State'));
echo(field('emp_zip',false,'text','10','Employer_Zip:Zip','class="zip"'));
echo(field('emp_phone',true,'text','13','Employer Phone:(000) 000-0000','class="phone"'));
echo(field('emp_phone_ext',false,'text','6','Employer Phone Ext:Ext'));
//echo(field('emp_fax',false,'text','13','Employer Fax'));
//echo(field('super_phone',false,'text','13','Supervisor Phone','class="phone"'));
echo(field('super_name',false,'text','100','Supervisor Name:Name'));
//echo(field('super_phone_ext',false,'number','','Supervisor Phone Ext'));
echo '</div>';
echo "<div class='formRow'>";
clear();
//echo(field('hire_date',false,'date','','Hire Date'));
left('Employement Type');right(field('emp_type',false,'Part Time|Full Time','','Employed Type'));
echo(field('job_title',true,'text','100','Job Title:Job Title'));
...
#       #     # #    #     #       #     #  #   #   #
#       #     # #     #    #        #####  ### #     #
echo "<div name='formpart6' id='formpart6' style='display:none;'>";
echo "<div class='formRow mobileExtraPad'>";
echo "<div id='p6namespace'></div>";
echo '</div>';
echo "<div class='formRow'>";
//echo(field('workshift',false,'O|T|S|F','','Workshift'));
//echo(field('gross_income',false,'number','','Gross Monthly Income'));
echo '</div>';
echo "<div class='formRow'>";
echo(field('net_income',true,'number','','Net Income:0','style="width:100px;"'));
echo '</div>';
echo "<div class='formRow'>";
clear();
left('Type of Income');right(field('income_type',true,'Employment|Disability|S.S.I.|Unemployment|Poo|Other','','Income Type'));
echo(field('employed_months',true,'number','','Number of Months Employed:0','style="width:100px;"'));
echo '</div>';
echo "<div class='formRow'>";
clear();
left('Pay Period');right(field('pay_period',true,'Weekly|Bi-Weekly|Semi-Monthly|Monthly','','Pay Period'));
echo '</div>';
echo "<div class='formRow'>";
clear();
left('Type of Payroll');right(field('pay_type',true,'Direct Deposit|Payroll/Paycheck','','Pay Type'));
echo '</div>';
echo "<div class='formRow'>";
clear();
echo(field('last_pay_date',true,'date','','Last Paydate'));clear();
echo(field('next_pay_date',true,'date','','Next Paydate'));clear();
echo(field('second_pay_date',true,'date','','Second Paydate'));clear();
echo("</div>");
echo "<!-- next prev buttons -->";
echo "<div style='margin-top: 20px; margin-bottom: 10px; width: 100%; text-align: center;'>";
btnprev('4');
...
#       #     # #    #     #       #     # #         # #   #       #    ##
#       #     # #     #    #        #####  #######    #    ####### #     #
echo "<div name='formpart7' id='formpart7' style='display:none;'>";
echo "<div class='formRow mobileExtraPad'>";
echo "<div id='p7namespace'></div>";
echo '</div>';
echo "<div class='formRow'>";
...
echo(field('bank_name',true,'text','50','Your Bank Name:Name of Bank'));
echo(field('bank_phone',false,'text','13','Your Bank Phone:(000) 000-0000','class="phone"'));
echo(field('account_holder',false,'text','100','Account Holder Name:Your Name'));
echo(field('routing_number',true,'text','9','Routing Number:000000000','class="rtn"'));
echo(field('acct_number',true,'text','17','Account Number:00000000'));
echo '</div>';
echo "<div class='formRow'>";
clear();
left("Type of Bank Account");right(field('account_type',true,'Checking Account|Savings Account','','Account Type'));
echo '</div>';
echo "<div class='formRow'>";
echo('How long have you had this account?<br/>');
left("Years: ".field('bank_years',false,'number','','Years','style="width:100px;"'));
right("Months: ".field('bank_months',true,'number','','Months','style="width:100px;"'));
echo "</div>";
...
#       #     # #    #     #       #        #  #     # #     #    #
#       #     # #     #    #       ####### ###  #####  #     #    #
echo "<div name='formpart8' id='formpart8' style='display:none;'>";
echo "<div class='formRow mobileExtraPad'>";
echo "<div id='p8namespace'></div>";
echo '</div>';
echo "<div class='formRow'>";
echo(field('ref_fname',false,'text','50','Reference First Name:First Name'));
echo(field('ref_lname',false,'text','50','Reference Last Name:Last Name'));
echo '</div>';
echo "<div class='formRow'>";
clear();
left('Reference Relationship (?)');right(field('ref_relation',false,'Spouse|Sibling|Parent|Co-Worker|Friend|Boyfriend|Girlfriend|Brother|Sister|Other','','Reference Relation (?)'));
echo '</div>';
echo "<div class='formRow'>";
echo(field('ref_phone',false,'text','13','Reference Phone:(000) 000-0000','class="phone"'));
echo("</div>");
clear();
echo "<!-- next prev buttons -->";
echo "<div style='margin-top: 20px; margin-bottom: 10px; width: 100%; text-align: center;'>";
...
#       #     # #    #     #               # 
#       #     # #     #    #        #######  
echo "<div name='formpart9' id='formpart9' style='display:none;'>";
echo "<div class='formRow mobileExtraPad'>";
echo "<h3>You are about to...</h3>";
echo "And you will also receive ...<br/>";
echo "And guess what else...";
echo '</div>';
echo "<div class='formRow mobileExtraPad'>";
echo(field('other_offers',false,'yesno','','Other Offers'));
echo '</div>';
echo "<div class='formRow mobileExtraPad'>";
echo(field('test',false,'yesno','','Test'));
echo("</div>");
echo "This is your last chance to check on data, offer to make any changes, maybe have a scroll-able \"Review\" (I don't really think that's necessary) or offer any kind of upsells or whatever.";
...
btnprev('7');
right("<input type='submit' name='submit' value='SUBMIT' class='btn-green' style='margin-bitton: 20px;'>");
...
<!-- footer centered -->
<div id='footer'>
© 2013 The Money Company &bull; Design: <a href='http://www.grafxtampa.com' target='_blank'>Grafx</a> &bull; Development: <a href='https://verswerks.com' target='_blank'>Fort Myers Web Design</a>
<br><br>
...
<b>xxxxxxxxxxxxxx</b>
<br><br>
We review each customer inquiry received through our customer support channels and do our best to respond to you in a timely manner.
</div>
<!-- END INNER CONTENT -->
...
<?php
function option($value,$text,$default)
	{
	$selected="";
	if ($value==$default) $selected='selected="selected"';
	xecho("<option value=\"$value\" $selected>$text</option>\n");
	}
function states($default)
	{
option("AL","Alabama",$default); 
option("AK","Alaska",$default); 
option("AZ","Arizona",$default); 
option("AR","Arkansas",$default); 
option("CA","California",$default); 
option("CO","Colorado",$default); 
option("CT","Connecticut",$default); 
option("DE","Delaware",$default); 
option("DC","District Of Columbia",$default); 
option("FL","Florida",$default); 
option("GA","Georgia",$default); 
option("HI","Hawaii",$default); 
option("ID","Idaho",$default); 
option("IL","Illinois",$default); 
option("IN","Indiana",$default); 
option("IA","Iowa",$default); 
option("KS","Kansas",$default); 
option("KY","Kentucky",$default); 
option("LA","Louisiana",$default); 
option("ME","Maine",$default); 
option("MD","Maryland",$default); 
option("MA","Massachusetts",$default); 
option("MI","Michigan",$default); 
option("MN","Minnesota",$default); 
option("MS","Mississippi",$default); 
option("MO","Missouri",$default); 
option("MT","Montana",$default); 
option("NE","Nebraska",$default); 
option("NV","Nevada",$default); 
option("NH","New Hampshire",$default); 
option("NJ","New Jersey",$default); 
option("NM","New Mexico",$default); 
option("NY","New York",$default); 
option("NC","North Carolina",$default); 
option("ND","North Dakota",$default); 
option("OH","Ohio",$default); 
option("OK","Oklahoma",$default); 
option("OR","Oregon",$default); 
option("PA","Pennsylvania",$default); 
option("RI","Rhode Island",$default); 
option("SC","South Carolina",$default); 
option("SD","South Dakota",$default); 
option("TN","Tennessee",$default); 
option("TX","Texas",$default); 
option("UT","Utah",$default); 
option("VT","Vermont",$default); 
option("VA","Virginia",$default); 
option("WA","Washington",$default); 
option("WV","West Virginia",$default); 
option("WI","Wisconsin",$default); 
option("WY","Wyoming",$default);
	}
function btnprev($n) { echo ('<div style="clear: both;"></div><a href="#" onclick="return handle('.$n.');" class="btn-green left" style="margin-bottom: 20px;">&#171; Previous</a>'); }
function btnnext($n) { echo('<div id="nextspace$n" name="nextspace$n" style="display: inline-block; margin: 0px auto; margin-top: 8px;">'.$n.' of 8</div><a href="#" onclick="return handle('.$n.');" class="btn-green right" style="margin-bottom: 20px;">Continue &#187;</a><div style="clear: both;"></div>'); }
